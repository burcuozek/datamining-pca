...
#rotation
pc_rot<-principal(USJudgeRatings, rotate = "varimax" )

#scores
pc<-principal(USJudgeRatings, nfactors = 2, scores =TRUE)
pc
head(pc$scores)
...
################################################################################################
library(readxl)
Glass_Identification_Data <- read_excel("Desktop/homework2/Glass Identification Data.xlsx")
View(Glass_Identification_Data)

#fa.parallel()
fa.parallel(Glass_Identification_Data)

#principal()
principal(Glass_Identification_Data)

#rotation
pc_rot<-principal(Glass_Identification_Data, rotate = "varimax" )
pc_rot

#scores
pc_2<-principal(Glass_Identification_Data, nfactors = 4 ,scores =TRUE)
pc_2
head(pc_2$scores)
...
#fa.parallel()
correlations <- cov2cor(Harman23.cor$cov)
fa.parallel(correlations)

#fa()
fa(correlations, nfactors = 2)

#rotate
fa(correlations, nfactors = 2, rotate = "varimax")
fa_promax <- fa(correlations,rotate = "promax" , fm = "pa")
fa_promax

#score
pc<-fa(correlations, nfactors = 2, rotate="varimax", score = TRUE)
pc
head(pc$weights)
...
Vertebral<- 
  read_excel("C:/Users/Masaüstü/IE7275/Vertebral Column Data.xlsx")

Vertebral <- Vertebral[,-7]

fa_vert <- fa.parallel(Vertebral)
fa_vert 

dist.vert <- dist(Vertebral)
cmd.vert <- cmdscale(dist.vert)
head(cmd.vert)

factor.plot(cmd.vert, title = "Orthogonal Scaling Graph")
